      ...
      if (First_Name === undefined || First_Name === '') {
        ...
        response.all(`SELECT * FROM USER WHERE Email =  '${Email}' COLLATE NOCASE`, (error, result, fields) => {
          if (error) {
            ...
            response.all(`INSERT INTO USER (ID, First_Name, Last_Name, Email, Password, Role) VALUES ('${uid}', '${First_Name}', '${Last_Name}', '${Email}', '${Password}', '${Role}')`,
              (error, result, fields) => {
                if (error) {
  ...
  connectDB((isSuccess, response) => {
    if (isSuccess) {
      ...
      if (ID === undefined || ID === '') {
        ...
        response.all(`SELECT * FROM USER WHERE ID = '${ID}'`, (error, result, fields) => {
          if (error) {
            res.status(400).send({ error: error })
          } else {
            if (result && result.length > 0) {
  ...
  connectDB((isSuccess, response) => {
    if (isSuccess) {
      ...
      if (ID === undefined || ID === '') {
        ...
        response.all(`UPDATE USER SET First_Name = '${First_Name}', Last_Name = '${Last_Name}', Email = '${Email}', Password = '${Password}', Role = '${Role}' WHERE ID = '${ID}'`,
          (error, result, fields) => {
            if (error) {
              res.status(400).send({ error: error })
            } else {
