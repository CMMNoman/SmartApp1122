      ...
      if (iid === undefined || iid === '') {
        ...
        response.query(`INSERT INTO ASSIGNED_AMBULANCE_HOSPITAL (IID, HID, AIDS) VALUES ('${iid}', '${hid}', '${aids}')`, (error, result, fields) => {
          if (error) {
            ...
            response.query(`UPDATE AMBULANCE SET STATUS = 'Assigned' WHERE ID IN (${aidsJoin})`)
...
router.get('/GetAll', (req, res, next) => {
  connectDB((isSuccess, response) => {
    if (isSuccess) {
      response.query(`SELECT * FROM USER WHERE ROLE = 'DISPATCHER'`, (error, result, fields) => {
        if (error) {
          response.destroy()
          res.status(400).send({ error: error })
        } else {
...
router.get('/Delete/:UID', (req, res, next) => {
  connectDB((isSuccess, response) => {
    if (isSuccess) {
      ...
      response.query(`DELETE FROM USER WHERE ROLE = 'DISPATCHER' AND ID = '${UID}'`, (error, result, fields) => {
        if (error) {
          response.destroy()
          res.status(400).send({ error: error })
        } else {
