            ...
            response.query(`INSERT INTO USER (ID, First_Name, Last_Name, Email, Password, Role) VALUES ('${uid}', '${Op_First_Name}', '${Op_Last_Name}', '${Op_Email}', '${Op_Password}', 'Ambulance')`,
              (error, result, fields) => {
                if (error) {
                  ...
                  response.query(`INSERT INTO AMBULANCE (ID, Name, Reg_No, Op_ID, Op_Phone_No, Location, Status) VALUES ('${aid}', '${Name}', '${Reg_No}', '${uid}', '${Op_Phone_No}', '${Location}', 'Available')`, (error, result, fields) => {
                    if (error) {
                      response.destroy()
                      res.status(400).send({ error: error })
                    } else {
  ...
  connectDB((isSuccess, response) => {
    if (isSuccess) {
      ...
      if (ID === '') {
        ...
        response.query(`UPDATE AMBULANCE SET Current_Location = '${Location}' WHERE Op_ID = '${ID}'`, (error, result, fields) => {
          if (error) {
            response.destroy()
            res.status(400).send({ error: error })
          } else {
...
router.post('/GetAmbulancesCurrentLocation', (req, res, next) => {
  connectDB((isSuccess, response) => {
    if (isSuccess) {
      ...
      response.query(`SELECT Current_Location FROM AMBULANCE WHERE ID IN (${aidsJoin}) ORDER BY ID`, (error, result, fields) => {
        if (error) {
          response.destroy()
          res.status(400).send({ error: error })
        } else {
...
router.get('/Delete/:AID', (req, res, next) => {
  connectDB((isSuccess, response) => {
    if (isSuccess) {
      ...
      response.query(`SELECT * FROM ASSIGNED_AMBULANCE_HOSPITAL WHERE AIDS LIKE CONCAT('%', '${AID}', '%')`, (error, result, fields) => {
        if (error) {
          response.destroy()
          res.status(400).send({ error: error })
        } else {
          if (result.length > 0) {
            ...
            response.query(`DELETE FROM USER WHERE ROLE = 'AMBULANCE' AND ID = (SELECT Op_ID FROM AMBULANCE WHERE ID = '${AID}')`)
            response.query(`DELETE FROM AMBULANCE WHERE ID = '${AID}'`, (error, result, fields) => {
              if (error) {
                response.destroy()
                res.status(400).send({ error: error })
              } else {
