...
router.get('/GetAll', (req, res, next) => {
  connectDB((isSuccess, response) => {
    if (isSuccess) {
      response.all(`SELECT * FROM INCIDENT`, (error, result, fields) => {
        if (error) {
...
router.get('/GetByID/:ID', (req, res, next) => {
  connectDB((isSuccess, response) => {
    if (isSuccess) {
      const { ID } = req.params
      response.all(`SELECT * FROM INCIDENT WHERE ID = '${ID}'`, (error, result, fields) => {
        if (error) {
        ...
        if (error) {
          ...
          response.all(`UPDATE AMBULANCE SET STATUS = 'Available' WHERE ID IN (${aidsJoin})`, (error, result, fields) => {
            if (error) {
              ...
              response.all(`UPDATE INCIDENT SET STATUS = 'Resolved' WHERE ID = '${IID}'`, (error, result, fields) => {
                if (error) {
