...
router.get('/Delete/:HID', (req, res, next) => {
  connectDB((isSuccess, response) => {
    if (isSuccess) {
      ...
      response.query(`SELECT * FROM ASSIGNED_AMBULANCE_HOSPITAL WHERE HID = '${HID}'`, (error, result, fields) => {
        if (error) {
          response.destroy()
          res.status(400).send({ error: error })
        } else {
          if (result.length > 0) {
            ...
            response.query(`DELETE FROM HOSPITAL WHERE ID = '${HID}'`, (error, result, fields) => {
              if (error) {
                response.destroy()
                res.status(400).send({ error: error })
              } else {
